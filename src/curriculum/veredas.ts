...
export type LicaoDeVereda =
  ...
  | {
    /*
      O Explorador de Arquivos, e por que ele é mais um tipo.

      Vale a conta já escrita em `ambiente` e em `terminal`: não há `modelo`
      string nem linguagem. O que se confere é o **disco** depois dos gestos —
      que pastas existem, o que está dentro delas, o que foi para a Lixeira e
      voltou, o que chegou inteiro no pen drive. Enfiá-lo em `laboratorio`
      ...
      O disco de partida não vem do currículo: é sempre o mesmo computador do
      clube, e ele mora em `discoDoClube()`. Um currículo que pudesse descrever
      o disco poderia descrevê-lo com a hierarquia já montada e o pacote já
      feito, que é justamente o laboratório que abre resolvido.
    ...
    */
    ...
    titulo: string;
    resumo: string;
    /** Os ids de verificação, no validador do Explorador. */
    verificacoes: string[];
  }
  | {
    /*
    ...
    */
     ...
     */
    documento: 'oficio' | 'circular' | 'relatorio' | 'anual' | 'revisao' | 'entrega';
