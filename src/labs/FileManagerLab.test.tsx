...
const linha = (nome: string) => todos('div.select-none').find(d => d.textContent?.includes(nome));

/** Uma pasta na árvore da esquerda. */
const naArvore = (nome: string) => todos('div.cursor-pointer').find(d => d.textContent?.trim() === nome);
...
const nomesVisiveis = () => todos('div.select-none').map(d => d.textContent?.split(/\d{2}\//)[0].trim());
...
describe('as seis operações do requisito', () => {
  it('cumpre as seis e só então libera a conclusão', () => {
    ...
    clicar(botao('Nova pasta'), 'Nova pasta');
    ...
    clicar(botao('Copiar'), 'Copiar');
    clicar(naArvore('Documentos'), 'Documentos na árvore');
    clicar(botao('Colar'), 'Colar');
    ...
    clicar(botao('Recortar'), 'Recortar');
    clicar(naArvore('Documentos'), 'Documentos na árvore');
    clicar(botao('Colar'), 'Colar');
...
describe('o que a janela recusa', () => {
  ...
  */
  ...
  it('numera o nome repetido, como o Windows', () => {
    clicar(botao('Nova pasta'), 'primeira');
    renomearPara('Trabalhos');
    clicar(botao('Nova pasta'), 'segunda');
    renomearPara('Trabalhos');
