...
const personagem = (id: string, nome: string, blocos: Bloco[][]): Personagem => ({
  id, nome, trajes: ['🐱'], x: 0, y: 0,
...
describe('o roteiro descreve a pilha', () => {
  it('diz quando ela roda, pelo chapéu', () => {
    const r = roteiroDeApresentacao(proj([personagem('gato', 'Gato', [
    ...
    expect(r[0].personagem).toBe('Gato');
  });
  ...
  */
  it('deixa de fora a pilha que não roda, e a conta à parte', () => {
    const p = proj([personagem('gato', 'Gato', [[b('mover', { passos: 10 })]])]);
