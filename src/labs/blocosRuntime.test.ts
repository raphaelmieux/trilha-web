...
import type { Bloco, Personagem, Projeto } from './blocos';
...
const personagem = (id: string, blocos: Bloco[][], x = 0, y = 0): Personagem => ({
  id, nome: id, trajes: ['🐱', '🐈'], x, y,
  pilhas: blocos.map((bs, i) => ({ id: `${id}-p${i}`, blocos: bs })),
});

const projeto = (personagens: Personagem[], variaveis: Projeto['variaveis'] = []): Projeto =>
  ({ personagens, variaveis });

describe('a bandeira verde', () => {
  it('só roda as pilhas que começam com o chapéu dela', () => {
    const p = new Palco(projeto([personagem('gato', [
      [b('quandoBandeira'), b('mover', { passos: 10 })],
      [b('quandoTecla', { tecla: 'direita' }), b('mover', { passos: 100 })],
    ])]));
    p.bandeira();
    p.rodar(3);
    expect(p.estado.personagens[0].x).toBe(10);
  });

  it('reinicia o palco: posição e variáveis voltam ao começo', () => {
    const p = new Palco(projeto(
      [personagem('gato', [[b('quandoBandeira'), b('mover', { passos: 50 })]])],
      [{ nome: 'placar', valor: 0 }],
    ));
    p.bandeira();
    p.rodar(3);
    p.estado.variaveis.placar = 99;
    expect(p.estado.personagens[0].x).toBe(50);

    p.bandeira();
    expect(p.estado.personagens[0].x).toBe(0);
...
describe('a repetição', () => {
  it('anda uma vez por quadro, e não tudo de uma vez', () => {
    ...
    /* Um quadro, um passo: o `mova` da primeira volta. Se andasse 50 de uma
       vez, o desbravador não veria o personagem se mexer — veria ele aparecer
...
describe('o sempre', () => {
  ...
  */
  ...
  it('continua rodando quadro após quadro', () => {
    ...
    p.bandeira();
    p.rodar(10);
    expect(p.estado.personagens[0].x).toBeGreaterThan(0);
    expect(p.estado.rodando).toBe(true);
  });
});

describe('a condição', () => {
  it('vê um personagem tocando o outro', () => {
    const p = new Palco(projeto(
      [
        personagem('gato', [[
          b('quandoBandeira'),
  ...
  it('compara uma variável com um número', () => {
    ...
    p.bandeira();
    p.rodar(5);
    expect(p.estado.personagens[0].fala).toBe('ganhei');
  });
});
...
describe('o palco tem beirada', () => {
  it('o personagem não sai da tela', () => {
    const p = new Palco(projeto([personagem('gato', [
      [b('quandoBandeira'), b('sempre', { corpo: [b('mover', { passos: 50 })] })],
    ])]));
    p.bandeira();
    p.rodar(60);
    expect(p.estado.personagens[0].x).toBe(240);
  });
  ...
  it('tocando na borda é verdade quando encosta nela', () => {
    ...
    p.bandeira();
    p.rodar(60);
    expect(p.estado.personagens[0].fala).toBe('cheguei');
  });
});
...
describe('dois personagens', () => {
  it('andam ao mesmo tempo, e não um depois do outro', () => {
    const p = new Palco(projeto([
      personagem('gato', [[b('quandoBandeira'), b('sempre', { corpo: [b('mover', { passos: 5 })] })]], -100, 0),
      personagem('cao', [[b('quandoBandeira'), b('sempre', { corpo: [b('mover', { passos: 5 })] })]], -100, 50),
    ]));
    p.bandeira();
    p.rodar(5);
    const [gato, cao] = p.estado.personagens;
    expect(gato.x).toBe(cao.x);
    expect(gato.x).toBeGreaterThan(-100);
  });
});

describe('o traje e o som', () => {
  it('o próximo traje dá a volta na lista', () => {
    const p = new Palco(projeto([personagem('gato', [
      [b('quandoBandeira'), b('proximoTraje'), b('proximoTraje')],
    ])]));
    p.bandeira();
    p.rodar(1); expect(p.estado.personagens[0].traje).toBe(1);
    p.rodar(1); expect(p.estado.personagens[0].traje).toBe(0);
  });
